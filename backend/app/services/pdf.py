...
def render_pdf_template(
    student_name: str, excursion: dict, parent_fields: dict, signature_png: Optional[bytes]
) -> bytes:
    buffer = BytesIO()
    ...
    c.drawString(
        2 * cm,
        height - 5 * cm,
        f"Дата: {excursion.get('date')} | Место: {excursion.get('location')}",
    )
    ...
    c.drawString(
        ...
        height - 7 * cm,
        (
            f"Родитель: {parent_fields.get('parent_name', '—')} | "
            f"Телефон: {parent_fields.get('parent_phone', '—')}"
        ),
    )
